// utils/skillUtils.ts
// Utility functions and data for the Skills section.
// Kept here rather than inline in the component so the color logic
// and SVG strings can be shared or updated in one place.

/**
 * Returns a hex color for a skill progress bar based on the skill's proficiency level.
 * Colors follow the portfolio accent palette:
 *   90+  → soft teal  (expert)
 *   80+  → muted gold (advanced)
 *   60+  → electric blue (intermediate)
 *   <60  → translucent blue (beginner)
 */
export const getSkillLevelColor = (level: number): string => {
  if (level >= 90) return '#20C997'; // Soft teal — expert
  if (level >= 80) return '#D4AF37'; // Muted gold — advanced
  if (level >= 60) return '#0077FF'; // Electric blue — intermediate
  return '#0077FF80';                // Translucent blue — beginner
};

/**
 * Inline SVG strings used as fallback icons for soft skills that have
 * no dedicated icon in the lucide-react library.
 * Keyed by the exact skill name string used in skillData.ts.
 */
